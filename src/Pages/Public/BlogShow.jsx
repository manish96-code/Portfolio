    ...
    return (
        <AppLayout settings={settings} socialLinks={socialLinks}>
            <article className="max-w-[768px] mx-auto pt-32 pb-24">
                <a href="#/blogs" className="inline-flex items-center gap-2 text-sm font-mono text-green hover:underline mb-8">
                    ← Back to Articles
                </a>

                <header className="space-y-4 mb-10 pb-10 border-b border-navy-lighter/30">
                    <div className="flex items-center gap-3 text-xs font-mono">
                        <span className="text-green">{blog.category}</span>
                        <span className="text-slate/40">•</span>
                        <span className="text-slate/60">{new Date(blog.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </div>

                    <h1 className="text-3xl md:text-4xl font-bold text-slate-lightest tracking-tight leading-tight">
                        {blog.title}
                    </h1>
                ...
                <div className="bg-navy-light border border-navy-lighter/30 p-8 md:p-12 rounded-xl">
